...
export class UVA {
    ...
    async create(data) {
        ...
        VALUES (?, ?, ?)
      `);
            const result = stmt.run(data.date, data.value, data.source || 'bcra');
    ...
    async batchUpsert(uvaValues) {
        if (uvaValues.length === 0)
            return 0;
        try {
            const transaction = this.db.transaction((valuesToInsert) => {
    ...
    async deleteOldUVAValues(beforeDate) {
        try {
     ...
     */
    async getInflationRates(fromDate, toDate, periodType = 'monthly') {
        try {
            const baseQuery = `
        SELECT date, value 
        FROM uva_values 
      ...
      `;
            ...
            const uvaValues = stmt.all(fromDate, toDate);
