...
export class Quote {
    ...
    async create(data) {
      ...
      `);
            const result = stmt.run(data.instrument_id, data.price, data.volume || null, data.high || null, data.low || null, data.close || data.price, // Default close to price if not provided
            data.quote_date, data.quote_time || null, data.source || 'yahoo_finance');
    ...
    async findByInstrumentId(instrumentId, limit = 100) {
        try {
            const stmt = this.db.prepare(`
        SELECT * FROM quotes 
        WHERE instrument_id = ? 
        ...
        LIMIT ?
      `);
            return stmt.all(instrumentId, limit);
        }
        catch (error) {
    ...
    async findBySymbol(symbol, limit = 100) {
        try {
            const stmt = this.db.prepare(`
        SELECT 
          q.*,
        ...
        LIMIT ?
      `);
            const results = stmt.all(symbol.toUpperCase(), limit);
    ...
    async getLatestQuote(instrumentId) {
        try {
            const stmt = this.db.prepare(`
        SELECT * FROM quotes 
        WHERE instrument_id = ? 
    ...
    async getLatestQuoteBySymbol(symbol) {
        try {
            const stmt = this.db.prepare(`
        SELECT 
          q.*,
    ...
    async upsertQuote(data) {
      ...
      `);
            const existing = existingStmt.get(data.instrument_id, data.quote_date);
            if (existing) {
          ...
          WHERE id = ?
        `);
                updateStmt.run(data.price, data.volume || null, data.high || null, data.low || null, data.close || data.price, data.quote_time || null, data.source || 'yahoo_finance', existing.id);
    ...
    async batchUpsert(quotes) {
        if (quotes.length === 0)
            return 0;
        try {
            const transaction = this.db.transaction((quotesToInsert) => {
    ...
    async deleteOldQuotes(beforeDate) {
        try {
    ...
    async getWatchlistQuotes(limit = 50) {
        try {
