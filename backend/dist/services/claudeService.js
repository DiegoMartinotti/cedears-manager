...
export class ClaudeService extends EventEmitter {
    ...
    async initialize() {
        try {
            ...
            throw new ClaudeServiceError('Failed to initialize Claude service', 'INIT_ERROR', error);
        }
    }
    /**
     * Verifica que Claude CLI esté disponible
     */
    async checkClaudeCliAvailability() {
    ...
    async analyze(request) {
        ...
        if (!this.isInitialized) {
            throw new ClaudeServiceError('Claude service not initialized', 'NOT_INITIALIZED');
        }
        try {
     ...
     */
    async executeClaudeCommand(request) {
        return new Promise((resolve, reject) => {
            ...
            claudeProcess.on('close', (code) => {
                if (code === 0) {
                    try {
                        ...
                        reject(new ClaudeServiceError('Failed to parse Claude response', 'PARSE_ERROR', { stdout, parseError }));
                    }
                }
                else {
                    reject(new ClaudeServiceError(`Claude CLI exited with code ${code}`, 'CLI_ERROR', { code, stderr, stdout }));
                }
            });
            claudeProcess.on('error', (error) => {
                reject(new ClaudeServiceError('Failed to execute Claude CLI', 'EXECUTION_ERROR', error));
            });
        });
    }
    /**
