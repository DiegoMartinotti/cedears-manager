...
export class UVAService {
    ...
    rateLimiter;
    // URLs de APIs
    BCRA_UVA_URL = 'https://www.bcra.gob.ar/PublicacionesEstadisticas/Principales_variables_datos.asp?serie=7913&detalle=Unidad%20de%20Valor%20Adquisitivo%20(UVA)%20(en%20pesos%20desde%20el%2031.3.17,%20base%2031.3.16=14.05%20pesos)';
    ESTADISTICAS_BCRA_API = 'https://api.estadisticasbcra.com/uva';
     ...
     */
    async fetchUVAFromEstadisticas() {
        const today = format(new Date(), 'yyyy-MM-dd');
        try {
            if (!await this.rateLimiter.checkLimit()) {
     ...
     */
    async updateHistoricalUVAValues(fromDate, toDate) {
        try {
     ...
     */
    async fetchHistoricalUVAFromEstadisticas(fromDate, toDate) {
        try {
            const response = await axios.get(this.ESTADISTICAS_BCRA_API, {
                timeout: 15000,
                params: {
     ...
     */
    async setCachedUVAValue(date, value) {
        try {
            const cacheKey = `uva_value_${date}`;
     ...
     */
    async cleanupOldUVAValues(keepDays = 365) {
        try {
...
//# sourceMappingURL=UVAService.js.map