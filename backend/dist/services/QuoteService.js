...
export class QuoteService {
    ...
    CACHE_TTL_MARKET_OPEN = 2 * 60 * 1000; // 2 minutos durante mercado abierto
    CACHE_TTL_MARKET_CLOSED = 30 * 60 * 1000; // 30 minutos fuera de horario
    CACHE_TTL_WEEKEND = 60 * 60 * 1000; // 1 hora los fines de semana
    constructor() {
     ...
     */
    async getBatchQuotes(symbols, forceRefresh = false) {
        ...
        if (symbolsToFetch.length > 0) {
            ...
            for (let i = 0; i < symbolsToFetch.length; i += chunkSize) {
                ...
                const chunkPromises = chunk.map(symbol => this.getQuote(symbol, true).catch(error => ({
                    success: false,
                    symbol,
                    error: error.message,
                    source: 'yahoo_finance'
     ...
     */
    async updateAllWatchlistQuotes() {
        try {
     ...
     */
    async getQuoteHistory(symbol, filters = {}) {
        try {
            const searchFilters = {
    ...
    async cleanupOldQuotes(daysToKeep = 30) {
        try {
            ...
            const deletedCount = await this.quoteModel.deleteOldQuotes(format(cutoffDate, 'yyyy-MM-dd'));
     ...
     */
    async saveQuoteToDatabase(instrumentId, quoteData) {
        try {
