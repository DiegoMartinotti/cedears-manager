...
export class DashboardService {
    ...
    async getDashboardSummary() {
        try {
            ...
            const [portfolioSummary, currentPositions, marketSummary, performanceMetrics] = await Promise.all([
    ...
    async getMarketSummary() {
        try {
            ...
            const sectorPerformance = await this.calculateSectorPerformance();
            const marketSummary = {
    ...
    async getDistributionData() {
        try {
            ...
            const byAsset = positions.map((position, index) => ({
                symbol: position.symbol,
                companyName: position.company_name,
            ...
            const bySector = Array.from(sectorMap.entries()).map(([sector, data], index) => ({
                sector,
                value: data.value,
            ...
            const byESGStatus = Array.from(esgMap.entries()).map(([category, data], index) => ({
                category: category,
                value: data.value,
                percentage: totalValue > 0 ? (data.value / totalValue) * 100 : 0,
                positionsCount: data.count,
