...
export class CommissionController {
     ...
     */
    async getCommissionConfigs(req, res) {
        try {
     ...
     */
    async saveCommissionConfig(req, res) {
        try {
    ...
    async calculateCommission(req, res) {
        try {
            ...
            // Calcular comisiones
            const operationCommission = this.commissionService.calculateOperationCommission(type.toUpperCase(), amount, config);
            let result = {
            ...
            if (portfolioValue !== undefined && portfolioValue >= 0) {
                const projection = this.commissionService.calculateCommissionProjection(type.toUpperCase(), amount, portfolioValue, config);
                result = projection;
            }
            res.status(200).json({
                success: true,
     ...
     */
    async analyzeCommissions(req, res) {
        try {
            ...
            if (instrumentId) {
                filters.instrumentId = parseInt(instrumentId);
            }
    ...
    async compareBrokers(req, res) {
        try {
            ...
            const comparison = this.commissionService.compareBrokerCommissions(operationType.toUpperCase(), operationAmount, portfolioValue);
            res.status(200).json({
                success: true,
                data: comparison,
     ...
     */
    async calculateMinimumInvestment(req, res) {
        try {
            ...
            const result = this.commissionService.calculateMinimumInvestmentForCommissionThreshold(commissionThreshold, config);
            res.status(200).json({
                success: true,
                data: result,
