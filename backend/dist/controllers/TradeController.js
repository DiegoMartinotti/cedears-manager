...
import { TradeCreateSchema, TradeUpdateSchema, TradeQuerySchema, TradeParamsSchema, TradeSummaryQuerySchema } from '../schemas/trade.schema.js';
...
export class TradeController {
    ...
    async calculateCommission(req, res) {
        try {
            ...
            const calculation = this.commissionService.calculateOperationCommission(type, total_amount, config);
     ...
     */
    async projectCommission(req, res) {
        try {
            ...
            const projection = this.commissionService.calculateCommissionProjection(type, operation_amount, portfolio_value_ars, config);
     ...
     */
    async analyzeTrade(req, res) {
        try {
            const { id } = TradeParamsSchema.parse(req.params);
            const { current_price } = req.query;
            const currentPriceNum = current_price ? parseFloat(current_price) : undefined;
     ...
     */
    async validateDiversification(req, res) {
        try {
            ...
            const validation = await this.tradeService.validateDiversification(instrument_id, purchase_amount);
     ...
     */
    async getTradesSummary(req, res) {
        try {
     ...
     */
    async getMonthlyTradesSummary(req, res) {
        try {
            const query = TradeSummaryQuerySchema.parse(req.query);
     ...
     */
    async getBrokerConfigurations(req, res) {
        try {
     ...
     */
    async compareBrokerCommissions(req, res) {
        try {
            ...
            const comparison = this.commissionService.compareBrokerCommissions(operation_type, operation_amount, portfolio_value_ars);
     ...
     */
    async getMinimumInvestmentRecommendation(req, res) {
        try {
            ...
            const recommendation = this.commissionService.calculateMinimumInvestmentForCommissionThreshold(commission_threshold_percentage, config);
