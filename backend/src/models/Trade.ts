...
export class Trade {
  ...
  async create(data: Omit<TradeData, 'id' | 'created_at'>): Promise<TradeData> {
    try {
  ...
  async findById(id: number): Promise<TradeData | null> {
    try {
      ...
      const result = stmt.get(id) as TradeData | undefined
      return result || null
    } catch (error) {
  ...
  async findAll(filters?: {
    ...
    try {
      ...
      return stmt.all(...params) as TradeData[]
    } catch (error) {
  ...
  async findAllWithInstruments(filters?: {
    ...
    try {
      ...
      const stmt = this.db.prepare(query)
      return stmt.all(...params) as TradeWithInstrument[]
    } catch (error) {
  ...
  async findByInstrumentId(instrumentId: number, limit?: number): Promise<TradeData[]> {
  ...
  async getTradesSummary(instrumentId?: number): Promise<{
    total_trades: number
    total_buys: number
    total_sells: number
  ...
  async getMonthlyTradesSummary(year?: number): Promise<Array<{
    month: string
    total_trades: number
    buy_amount: number
    sell_amount: number
