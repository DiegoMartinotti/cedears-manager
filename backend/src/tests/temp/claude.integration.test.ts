...
import { claudeService, ClaudeService, ClaudeServiceError } from '../services/claudeService.js'
import { claudeAnalysisService, ClaudeAnalysisService } from '../services/claudeAnalysisService.js'
...
describe('Claude Integration Tests', () => {
  ...
  describe('ClaudeAnalysisService Integration Tests', () => {
    let analysisService: ClaudeAnalysisService
      ...
      for (let i = 0; i < config.maxConcurrentRequests + 2; i++) {
        promises.push(
          analysisService.analyze(
            { prompt: `Test ${i}`, instrumentCode: `TEST${i}` },
  ...
  describe('Error Handling Tests', () => {
    it('should handle service not initialized error', async () => {
      ...
      await expect(
        analysisService.analyze({ prompt: 'test without init' })
...
describe('Real Claude CLI Connection Test', () => {
  it.skip('should connect to real Claude CLI if available', async () => {
    ...
    try {
      ...
      console.log('❌ Could not test real Claude CLI:', (error as Error).message)
