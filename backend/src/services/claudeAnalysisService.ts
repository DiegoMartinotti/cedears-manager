import { claudeService, ClaudeAnalysisRequest, ClaudeAnalysisResponse, ClaudeServiceError } from './claudeService.js'
...
export class ClaudeAnalysisService {
  ...
  async analyze(
    ...
    if (!this.isInitialized) {
      throw new ClaudeServiceError(
        'Claude Analysis Service not initialized',
        'NOT_INITIALIZED'
      )
    }
    ...
    const cacheKey = useCache ? cacheService.generateAnalysisKey(
      request.prompt,
  ...
  async quickAnalysis(
    ...
    return this.analyze(
      { prompt, instrumentCode },
      { useCache: true, cacheTTLMinutes: 10, retryAttempts: 1 }
    )
  }
   ...
   */
  private calculateWaitTime(rateLimitStatus: any): number {
